...
# ===============================
classnames = []
with open(os.path.join(DATA_ROOT, "kitti_closest_vehicle_distance.txt"), "r", encoding="utf-8") as f:
    for line in f:
        line = line.strip()
        if line:
            classnames.append(line)

print(f"📚 Loaded {len(classnames)} KITTI class descriptions:")
...
if __name__ == "__main__":
    SPLIT_TXT = "test.txt"  # 也可以改为 train800.txt / val200.txt 等
    results = []
    ...
    out_csv = f"kitti_zero_shot_results_{SPLIT_TXT.replace('.txt','')}.csv"
